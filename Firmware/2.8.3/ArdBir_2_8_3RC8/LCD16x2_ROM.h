char *PIDName[]   ={"Uz  ", "Constanta P", "Constanta I", "Constanta D", "SampleTime ", "Interval ms", "In fierbere", "Calibrare  ", "Histereza  "};
char *stageName[] ={"Mash In   ", "Fitaza    ", "Glucanaza ", "Proteaza  ", "\xE2""Amilaza  ", "\xE0""Amilaza1 ", "\xE0""Amilaza2 ", "Mash Out  ", "Fierbere  ", "Fierbere  ", "Agitare   "};
char *unitName[]  ={"Scara Grade  ", "Senzor   ", "Fierbere    ", "Fierbere    ", "Ciclu Pompa  ", "Pauza Pompa   ", "Pmp PreMash", "Pmp la Mash", "Pmp MashOut", "PmpFierbere", "Stop Pompa  ", "Stop Pompa  ", "PID Pipe  " , "Skip Adauga   ", "Skip Elimina", "Skip Iod    ", "Timp Iod ", "Agitare  "};
...
void LCD_Procedo(){
  lcd.setCursor(1,1);
  lcd.print(F("Confirm? Da Nu "));
}

/*
void LCD_Conferma(){
  lcd.setCursor(1,1);
  lcd.print(F("Confirm? Da -- "));
}
*/
...
void Credits(){
  ...
  display_lcd(0,0,"Idee  Originala:",750);
  ...
  display_lcd(2,0,"Traduceri:",750);
  display_lcd(2,1," A. Moiseyev ",999);//Rusa
  display_lcd(2,1," A. Mondejar ",999);//Spaniola
  display_lcd(2,1," C.M. Macedo ",999);//Portugheza 20x4
  display_lcd(2,1,"F.A. Oliveira",999);//Portugheza 16x2
  display_lcd(2,1,"G.  Santaella",999);//Portugheza 20x4 (new)
  display_lcd(2,1,"  E. Eduard  ",999);//Norvegiana  20x4 (new)
  display_lcd(2,1," F.V. Paliuc ",999);//Romana  16x2 (new)
...
void PumpPrime(){
  lcd.setCursor(0,1);
  lcd.print(F("  Activez Pompa "));  // priming the pump
}  
